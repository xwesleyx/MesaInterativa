    ...
    try {
        ...
        if (req.user.role !== 'gm') {
             ...
             if (check.rows.length === 0 || check.rows[0].status === 'closed') {
                 throw new Error('Permissão negada. A mesa está fechada ou não existe.');
             }
             // Players continuam para salvar seus tokens...
        }

        // Criar ou Atualizar Sessão
        const checkSess = await client.query('SELECT id FROM game_sessions WHERE id = $1', [sId]);
        if (checkSess.rows.length > 0) {
            // Atualiza timestamp para forçar refresh nos outros clientes
            await client.query(`
                UPDATE game_sessions 
                SET name=$1, map_url=$2, status=$3, active_image_id=$4, active_video_id=$5, updated_at=NOW() 
                WHERE id=$6`, 
                [name, mapUrl, status, activeImageId, activeVideoId, sId]);
        } else {
            if (req.user.role !== 'gm') throw new Error('Apenas GM pode criar novas mesas.');
            await client.query(`
                INSERT INTO game_sessions (id, name, gm_id, map_url, status, active_image_id, active_video_id, updated_at) 
                VALUES ($1, $2, $3, $4, $5, $6, $7, NOW())`, 
                [sId, name, req.user.id, mapUrl, status, activeImageId, activeVideoId]);
        }

        // Limpar dados antigos (Método Simples: Delete All + Insert All para garantir sincronia total)
        // Em produção, um UPSERT seria mais eficiente, mas isso garante integridade rápida.
